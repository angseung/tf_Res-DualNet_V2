...
def dualpath_block(x, planes, stride=1, downsample=None, name=None):
    identity = x
...
def make_layer(x, planes, blocks, stride=1, name=None):
    ...
    x = dualpath_block(x, planes, stride, downsample, name=f"{name}.0")
    for i in range(1, blocks):
        x = dualpath_block(x, planes, name=f"{name}.{i}")

    return x


def resdualnetv1(x, blocks_per_layer, num_classes=10):
...
if __name__ == "__main__":
    ...
    model = resdualnet_v1(x, num_classes=10)
