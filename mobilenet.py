...
def basic_block(x, planes, stride=1, downsample=None, name=None):
    ...
    out = dconv3x3(x, stride=stride, name=f"{name}.dconv1_1")
    out = layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name=f"{name}.bn1")(out)
    out = layers.ReLU(name=f"{name}.relu1")(out)

    out = pconv(x, out_planes=planes, stride=stride, name=f"{name}.pconv1_1")
    out = layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name=f"{name}.bn1")(out)
    out = layers.ReLU(name=f"{name}.relu1")(out)
...
def mobilenetv1(x, blocks_per_layer, num_classes=10):
...
if __name__ == "__main__":
    ...
    model = mobilenet_v1(x, num_classes=10)