...
def basic_block(x, planes, stride=1, downsample=None, name=None):
    ...
    out = pconv(x, out_planes=planes, stride=1, name=f"{name}.pconv1_1")
    out = layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name=f"{name}.bn1")(out)
    out = layers.ReLU(name=f"{name}.relu1")(out)

    out = dconv3x3(out, stride=stride, name=f"{name}.dconv1_1")
    out = layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name=f"{name}.bn1")(out)
    out = layers.ReLU(name=f"{name}.relu1")(out)

    out = pconv(out, out_planes=out.shape[3], stride=1, name=f"{name}.pconv1_1")
    out = layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name=f"{name}.bn1")(out)
...
def shufflenet_v1(x, **kwargs):
    return shufflenetv1(x, [2, 2, 2, 2], **kwargs)


if __name__ == "__main__":
    x = np.random.randn(1, 32, 32, 3)
    x = tf.convert_to_tensor(x, dtype=np.float64)
    model = shufflenet_v1(x, num_classes=10)
